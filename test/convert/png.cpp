...
bool Png::save(const char* fileName) const
{
    ...
    png_set_IHDR(png_ptr, info_ptr, width, height,
                 ...
                 PNG_COMPRESSION_TYPE_BASE, PNG_FILTER_TYPE_BASE); // no return
...
uint32_t Png::getPixel(unsigned x, unsigned y) const
{
    ...
    switch (color_type)
    {
        case PNG_COLOR_TYPE_RGB:  return (*(uint32_t*)&row_pointers[y][x*3]) & 0xFFFFFF;
        case PNG_COLOR_TYPE_RGBA: return (*(uint32_t*)&row_pointers[y][x*4]) & 0xFFFFFF;
    }
    return 0;
}
